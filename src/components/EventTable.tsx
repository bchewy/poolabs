...
export function EventTable({ events }: EventTableProps) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-lg dark:border-white/5 dark:bg-zinc-900/80">
      <table className="min-w-full divide-y divide-white/10 text-sm">
        ...
        <tbody className="divide-y divide-white/5 text-zinc-700 dark:divide-white/10 dark:text-zinc-200">
          {events.map((event) => {
            ...
            return (
              <tr key={event.id} className="transition hover:bg-white/10 hover:backdrop-blur-sm dark:hover:bg-white/5">
                ...
                <td className="px-6 py-5 align-top space-y-2">
                  {event.flags.length === 0 ? (
                    ...
                    event.flags.map((flag) => (
                      <FlagBadge key={flag} label={flag} intent={flagIntent(flag)} />
                    ))
                  )}
                </td>
