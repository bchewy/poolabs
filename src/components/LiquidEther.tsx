...
export default function LiquidEther({
  ...
  useEffect(() => {
    setIsMounted(true);
    console.log('LiquidEther: Component mounted, THREE available:', typeof THREE !== 'undefined');
    return () => {
      setIsMounted(false);
      setIsInitialized(false);
      console.log('LiquidEther: Component unmounted');
    };
  }, []);

  // Additional initialization trigger
  useEffect(() => {
    if (isMounted && mountRef.current && !isInitialized) {
  ...
  useEffect(() => {
    console.log('LiquidEther: WebGL init effect started, mountRef.current:', mountRef.current, 'isInitialized:', isInitialized);
    if (!mountRef.current || !isInitialized) {
      console.warn('LiquidEther: mountRef.current or isInitialized is false', { mountRef: !!mountRef.current, isInitialized });
      return;
    }
...
`;
    ...
    }, 100); // Close the initTimer setTimeout

    return () => {
      clearTimeout(initTimer); // Clear the timeout on cleanup
  ...
  if (!isMounted) {
    return (
      <div
        ...
        style={{ ...style, width: '100%', height: '100%', position: 'relative', background: 'transparent' }}
      >
        <div style={{
          position: 'absolute',
          top: 0,
